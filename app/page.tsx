...
import { readBlog } from "@/lib/admin/apis";
    ...
    <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-5 p-5  sm:px-tablet">
      {blogs?.map((blog, index) => {
        return (
          <Link
            href={"/blog/" + blog.id}
            className="w-full  border rounded-md dark:bg-graident-dark p-5 hover:ring-2 ring-primary transition-all cursor-pointer space-y-5 first:lg:col-span-2 first:md:col-span-3"
            key={index}
          >
